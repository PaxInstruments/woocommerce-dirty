...
$dirty_order_header = array("order_number","ship_service","ship_name","ship_email","ship_address","ship_address2","ship_city","ship_province","ship_country","ship_postcode","ship_phonecode","ship_phone","bill_name","bill_email","bill_address","bill_address2","bill_city","bill_province","bill_country","bill_postcode","bill_phonecode","bill_phone","tax_id","shipping_price");
...
$dirty_order_product = array ("sku", "quantity", "price");
...
function dirty_filler() {
    ...
    print "<h2>Import processed orders</h2>";
    ?>
...
if (!empty($dirty_import_results)) {
    ...
    //global $wpdb;
    //$results = $wpdb->get_results( "SELECT * FROM wp_options WHERE option_name like '_wc_session_%';", OBJECT );

    //print_r($results);
    /*
    function explode_wp_option_value($option_value){
    $uns = maybe_unserialize($option_value);
    if(!is_array($uns)){
    return $uns;
    }
    else{
    foreach ($uns as $key => $value) {
    $uns[$key] = explode_wp_option_value($value);
    }
    }
    ...
    print "option_name:".$row->option_name."\n";
    print "option_value:\n";
    print_r(explode_wp_option_value($row->option_value));
    print "\n";
    //$us = maybe_unserialize($row->option_value);
    #print_r($us);
    #preg_replace('!s:(\d+):"(.*?)";!e', "'s:'.strlen('$2').':\"$2\";'", $row->option_value)
    #$us = unserialize();

    }
     */
    #$test_options = get_option('woocommerce_awd_shipping2_settings');
    #if(!$test_options){
    #print "ERROR";
    #update_option('woocommerce_awd_shipping2_settings', 'error');

    #}
    #update_option('paxmanchris_debug', $test_options);
    $pax_settings = get_option('paxmanchris_debug');

    #update_option('woocommerce_awd_shipping2_settings', get_option('paxmanchris_debug'));
    #$stype = gettype($test_options);
    #print "\nTEST: $stype\n";
    #print_r($test_options);
    //print_r($pax_settings);
    html_show_array($pax_settings);
    #print "<table border=1><tr>";
    #foreach ($pax_settings as $value) {
    #      print "<td>$value</td>";
    #}
    #print "</tr></table>";
    update_option('paxmanchris_debug', array());
    #print "<textarea>";
    #print serialize($pax_settings);
    #print "</textarea>";
    #print "\n";
    #print serialize(array('a'=>1,'b'=>2));
...
function get_dirty_order_data() {
    ...
    #$dopc = 0; # no longer needed
    $max_number_of_items = 0 ; #important for the cvs header
    ...
    while ($orders->have_posts()) {
        ...
        # order_number  ship_service    ship_name   ship_email  ship_address    ship_address2   ship_city   ship_province   ship_country    ship_postcode   ship_phonecode  ship_phone  bill_name   bill_email  bill_address    bill_address2   bill_city   bill_province   bill_country    bill_postcode   bill_phonecode  bill_phone  tax_id  shipping_price  
        #( sku quantity    price)
        $ship = array( $order_id,
            $shipping_method,
            ...
            '',  # shipping phone code
            $meta['_billing_phone'][0], #shipping phone
            $meta['_billing_first_name'][0] . " " . $meta['_billing_last_name'][0],
            $meta['_billing_email'][0],
            $meta['_billing_address_1'][0],
            $meta['_billing_address_2'][0],
            $meta['_billing_city'][0],
            $meta['_billing_state'][0],
            $meta['_billing_country'][0],
            $meta['_billing_postcode'][0],
            '', #billing phone code
        ...
        #  $items is a ugly list of all items like this:
        # Qty, Price, Name, Model, Attributes
        /* example output of items
            [0] => 1
            [1] => 165
            [2] => T400 Temperature Datalogger Set
            [3] => 927
            [4] => []
            [5] => 1
            [6] => 5
            [7] => Stainless Steel Thermocouple Probe
            [8] => 2530
            [9] => []

        Each 5 items is an item on the order . Dividing by 5 below allows
        to loop through each item later
        */        
        #if (count($items) / 5 > $dopc) {
        #    $dopc = count($items) / 5;
        #}
        ...
        for ($i=0; $i < count($items) ; $i+=5) { 
            $r_item = array( $items[$i+3], $items[$i+0], $items[$i+1]);
            $product_list = array_merge($product_list , $r_item);

        }

        if (abs(count($items)/5) > $max_number_of_items){
            $max_number_of_items = abs(count($items)/5);   
        }
        

        array_push($all_items, array_merge($ship, $product_list));
        

    }

    # insert the proper amount of item header
    for ($i=0; $i < $max_number_of_items ; $i++) { 
        $all_items[0] = array_merge($all_items[0], $dirty_order_product);
    }
    ...
    /* numbering of products on the header row no longer needed
    for ($i = 0; $i < $dopc; $i++) {
        $dop = $dirty_order_product;
        foreach ($dop as $key => $value) {
            $dop[$key] = str_replace("#", $i, $value);
